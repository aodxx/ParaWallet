...
The production baseline uses Google OpenID Connect ID tokens. `Auth.requireUser()` verifies issuer, audience, expiry, subject, verified email, active User status, role, and garden membership before protected work. The current source target is backend `2026.08.30-ocr-provider-v8` with schema `2026-08-production-v3`; see [`INDEX.md`](INDEX.md) for the deployed baseline, current acceptance evidence, and historical reports.
