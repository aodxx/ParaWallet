...
The production baseline uses Google OpenID Connect ID tokens. `Auth.requireUser()` verifies issuer, audience, expiry, subject, verified email, active User status, role, and garden membership before protected work. The current release is backend `2026.08.29-ux-ws3-v7` with schema `2026-08-production-v3`; see [`INDEX.md`](INDEX.md) for current acceptance evidence and historical reports.
