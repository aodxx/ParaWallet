...
3. กด Save และรัน `previewParaWalletProductionSchemaRepair()` ก่อน ฟังก์ชันนี้เป็น read-only และต้องรายงานเฉพาะ `correct` หรือ `known_legacy`; หากพบ `unexpected` ให้หยุดและห้ามแก้ header ด้วยมือ
4. รัน `repairParaWalletProductionSchema()` หนึ่งครั้ง ฟังก์ชันจะข้ามชีตที่ถูกต้อง และสร้าง backup ก่อนย้ายค่าใน `Agreements`, `Gardens`, `Buyers`, `Sales` และ `Settlements` ที่ยังเป็น legacy schema
5. Deploy Web App version ใหม่ แล้วเรียก `diagnostics.get`; ต้องได้ `release=2026.08.24-phase-d2`, `schemaVersion=2026-08-production-v3` และ `financialSchemaReady=true`
6. รัน `previewAuthorizedE2ETest()` เพื่อตรวจสวนป่าพะยอม, Agreement 60/40 และยืนยันว่า `existingSale`/`existingSettlement` ยังว่าง
7. หาก preview ถูกต้องและได้รับอนุมัติธุรกรรมทดสอบแล้ว ให้รัน `runAuthorizedE2ETestOnce()` เพียงครั้งเดียว
8. รัน `getAuthorizedE2ETestResult()` และเปิด Execution log เพื่ออ่านผลสรุป หากสำเร็จจะมีสถานะ `completed` และผลลัพธ์ถูกเก็บใน Script Properties

## การป้องกันความเสียหาย

ก่อนสร้าง Agreement หรือ Sale runner จะตรวจ critical production schema ทั้งชุด หากยังไม่ถูกซ่อมจะหยุดด้วย `E2E_PRODUCTION_SCHEMA_REPAIR_REQUIRED` ก่อนเขียนข้อมูล ฟังก์ชันนี้ไม่รับข้อมูลจาก HTTP request, ใช้ Script Lock, ตรวจสอบ Garden/Users/GardenMembers ก่อนทำรายการ, ใช้ request IDs คงที่สำหรับ test run, ตรวจสอบรายการเดิมก่อนเพิ่มซ้ำ และตั้งสถานะ `PARAWALLET_E2E_PAHPAYOM_STATUS=completed` หลังตรวจสอบหลักฐานครบเท่านั้น หากรันสำเร็จแล้ว การรันซ้ำจะหยุดด้วย `E2E_ALREADY_COMPLETED`
