  ...
  return <div className="app-shell">
    ...
    <div className="layout">
      ...
      <main className={`content ${loading ? "is-loading" : ""}`}>
        ...
        {screen === "gardens" && <GardensScreen garden={activeGarden} gardens={gardens.length ? gardens : activeGarden ? [activeGarden] : []} members={members} role={role} onCreate={() => setShowGardenForm(true)} onAddMember={() => setShowMemberForm(true)} onDeactivate={async (member) => { if (!activeGarden || !window.confirm(`ถอด ${member.name || member.email || "คนกรีด (Tapper)"} ออกจากสวนนี้หรือไม่\n\nระบบจะอนุญาตเมื่อไม่มีข้อตกลง รายการค้าง หรือยอดเงินคงค้างเท่านั้น`)) return; try { await api.members.deactivate({ gardenId: activeGarden.id, memberId: member.id }); await refresh("gardens"); setMessage("ปิดสิทธิ์คนกรีด (Tapper) ในสวนแล้ว โดยยังเก็บประวัติเดิมไว้ครบถ้วน"); } catch (caught) { setMessage(userMessageForApiError(caught)); } }} />}
  ...
  return <div className="tapper-overview">
    ...
    <section className="tapper-action-center" aria-label="งานหลักของคนกรีด">
      ...
      <div className="tapper-secondary-actions">
        ...
        <button onClick={onReports}><FileDown size={21} /><span>รายงานและส่งออก CSV</span></button>
      </div>
    </section>
    <section className="tapper-summary-grid"><Metric label="รายการกำลังรอเจ้าของสวน (Owner) ยืนยัน" value={connected ? `${data.pendingReviews} รายการ` : "—"} icon={<FileText />} /><Metric label="ยอดขายเดือนนี้" value={showMoney(data.monthlySales)} icon={<Sprout />} /></section>
...
function GardensScreen({ garden, gardens, members, role, onCreate, onAddMember, onDeactivate }: { garden?: Garden; gardens: Garden[]; members: GardenMember[]; role: Role; onCreate: () => void; onAddMember: () => void; onDeactivate: (member: GardenMember) => void }) {
  return <section className="panel"><div className="panel-head"><div><h2>สวนและแปลง</h2><p>จัดการพื้นที่ จำนวนต้นยาง และสมาชิกที่เข้าถึงข้อมูลสวน</p></div>{role === "owner" && <button className="primary" onClick={onCreate}><Plus size={16} />เพิ่มสวน</button>}</div>{gardens.length === 0 ? <Empty text="ยังไม่มีสวนที่เชื่อมต่อกับบัญชี" /> : <div className="card-grid">{gardens.map((item) => <div className="info-card" key={item.id}><span className="eyebrow">สวนที่ใช้งานอยู่</span><h3>{item.name}</h3><p>{item.province || "ไม่ระบุจังหวัด"} · {item.district || "ไม่ระบุอำเภอ"}</p><strong>{item.areaRai || 0} ไร่ · {(item.treeCount || 0).toLocaleString()} ต้น</strong></div>)}</div>}{role === "owner" && garden && <section className="member-management" aria-label="จัดการสมาชิกสวน"><div className="member-management-head"><div><span><Users size={19} />สมาชิกสวน</span><small>เฉพาะเจ้าของสวน (Owner) เท่านั้นที่เพิ่มหรือปิดสิทธิ์คนกรีด (Tapper) ได้</small></div><button className="secondary" onClick={onAddMember}><UserPlus size={16} />เพิ่มคนกรีด (Tapper)</button></div>{members.length === 0 ? <Empty text="ยังไม่มีสมาชิกที่ใช้งานอยู่ในสวนนี้" /> : <div className="member-list">{members.map((member) => <article className="member-row" key={member.id}><div className="member-avatar">{member.role === "owner" ? "O" : "T"}</div><div><strong>{member.name || (member.role === "owner" ? "เจ้าของสวน (Owner)" : "คนกรีด (Tapper)")}</strong><span>{member.email || member.userId}</span><small>{member.role === "owner" ? "เจ้าของสวน (Owner)" : "คนกรีด (Tapper) · ใช้งานอยู่"}</small></div>{member.role === "tapper" && <button className="danger-button" onClick={() => onDeactivate(member)}><UserMinus size={15} />ปิดสิทธิ์</button>}</article>)}</div>}</section>}</section>;
}

function AgreementsScreen({ agreements, garden, role, onCreate }: { agreements: Agreement[]; garden?: Garden; role: Role; onCreate: () => void }) { return <section className="panel"><div className="panel-head"><div><h2>ข้อตกลงแบ่งรายได้</h2><p>{garden?.name || "เลือกสวนก่อนสร้างข้อตกลง"} · ทุกเวอร์ชันไม่กระทบรายการย้อนหลัง</p></div>{role === "owner" && <button className="secondary" onClick={onCreate}>สร้างเวอร์ชันใหม่</button>}</div>{agreements.length === 0 ? <Empty text="ยังไม่มีข้อตกลงที่ใช้งานอยู่" /> : <div className="data-list">{agreements.map((agreement) => <article className="data-row" key={agreement.id}><div><strong>เวอร์ชัน {agreement.version}</strong><span>มีผล {agreement.effectiveFrom} · {agreement.status}</span></div><div><strong>{agreement.ownerPercentage}/{agreement.tapperPercentage}</strong><span>เจ้าของสวน (Owner) / คนกรีด (Tapper)</span></div></article>)}</div>}</section>; }
      ...
      {settlements.length === 0 ? <Empty text="ยังไม่มีรายการส่งเงินในช่วงนี้" /> : <div className="data-list">{settlements.map((item) =>
        <article className="data-row settlement-row" key={item.id}>
          <div className="settlement-row-copy">
            ...
            <small className="evidence-label">{item.method === "bank_transfer" ? <><FileText size={13} />{item.slipFileId ? "มีสลิปแนบ" : "ไม่พบสลิป"}</> : <><Banknote size={13} />รอเจ้าของสวน (Owner) ยืนยันการรับเงินสด</>}</small>
          </div>
...
function ReceiptForm({ garden, agreement, initialFile, onClose, onSaved }: { garden?: Garden; agreement?: Agreement; initialFile?: File | null; onClose: () => void; onSaved: () => void }) {
  ...
  const handleFile = async (selected: File | null) => {
    ...
    try {
      ...
      setFields((current) => ({ ...current, ...Object.fromEntries(Object.entries(next).filter(([key]) => !["receiptType", "needsReview"].includes(key)).map(([key, value]) => [key, String(value ?? "")])) }));
  ...
  const normalizedPreview = normalizeOcrFields({ ...fields, receiptType });
  const mathCheck = validateReceiptMath(normalizedPreview);
  const setReceiptTypeFromUser = (value: string) => setReceiptType(value as "weigh_ticket" | "rubber_form" | "unknown");
  return <Modal title="สแกนบิลและตรวจสอบ OCR" onClose={onClose}><div className="form-grid"><div className="receipt-upload-actions"><label className="file-action secondary"><Camera size={18} />ถ่ายภาพใบเสร็จ<input type="file" accept="image/*" capture="environment" onChange={(event) => void handleFile(event.target.files?.[0] || null)} /></label><label className="file-action secondary"><Upload size={18} />เลือกภาพจากเครื่อง<input type="file" accept="image/*" onChange={(event) => void handleFile(event.target.files?.[0] || null)} /></label></div>{file && <div className="calculation-preview"><span>ไฟล์: {file.name}</span><strong>{busy ? "กำลังอ่าน OCR..." : `สถานะ ${status}`}</strong><small>รูปแบบ: {receiptTypeLabel(receiptType)}</small></div>}<label>รูปแบบใบเสร็จ<select value={receiptType} onChange={(event) => setReceiptTypeFromUser(event.target.value)}><option value="unknown">ยังไม่ทราบรูปแบบ — เลือกเองได้</option><option value="weigh_ticket">ใบชั่งน้ำหนัก / บิลเงินสด</option><option value="rubber_form">แบบฟอร์มน้ำยาง–เปอร์เซ็นต์–ยางแห้ง</option></select><small className="form-hint">หากระบบจำแนกไม่ได้ ให้เลือกประเภทที่ตรงกับภาพเพื่อแสดงช่องข้อมูลที่ถูกต้อง</small></label><label>วันที่ขาย<input type="date" value={fields.saleDate} onChange={(e) => setField("saleDate", e.target.value)} /></label><label>ร้านรับซื้อ<input value={fields.buyerName} onChange={(e) => setField("buyerName", e.target.value)} /></label><label>ประเภทสินค้า<input value={fields.productType} onChange={(e) => setField("productType", e.target.value)} /></label><label>น้ำหนักสุทธิ (กก.)<input type="number" step="0.01" value={fields.weightKg} onChange={(e) => setField("weightKg", e.target.value)} /></label>{receiptType === "rubber_form" && <><label>น้ำหนักยางสด (กก.)<input type="number" step="0.01" value={fields.freshWeightKg || ""} onChange={(e) => setField("freshWeightKg", e.target.value)} /></label><label>เปอร์เซ็นต์เนื้อยาง (%)<input type="number" step="0.01" value={fields.drc || ""} onChange={(e) => setField("drc", e.target.value)} /></label><label>น้ำหนักยางแห้ง (กก.)<input type="number" step="0.01" value={fields.dryWeightKg || ""} onChange={(e) => setField("dryWeightKg", e.target.value)} /></label></>}<label>ราคา/กก.<input type="number" step="0.01" value={fields.unitPrice} onChange={(e) => setField("unitPrice", e.target.value)} /></label><label>ยอดก่อนหัก<input type="number" step="0.01" value={fields.grossSale} onChange={(e) => setField("grossSale", e.target.value)} /></label><label>รายการหัก<input type="number" step="0.01" value={fields.buyerDeductions} onChange={(e) => setField("buyerDeductions", e.target.value)} /></label>{fields.ocrError && <div className="calculation-preview low-confidence" role="alert"><span>สถานะผู้ให้บริการ OCR</span><strong>{fields.ocrError === "PROVIDER_NO_CANDIDATE" ? "ผู้ให้บริการไม่ส่งผลอ่านกลับมา กรุณาลองภาพใหม่" : fields.ocrError === "PROVIDER_INVALID_JSON" ? "ผู้ให้บริการส่งผลลัพธ์ที่อ่านไม่ได้ กรุณาตรวจข้อมูลด้วยตนเอง" : fields.ocrError.indexOf("PROVIDER_HTTP_") === 0 ? "ผู้ให้บริการ OCR ตอบข้อผิดพลาด กรุณาตรวจการตั้งค่ารุ่นและสิทธิ์ API" : "ระบบ OCR ต้องตรวจสอบเพิ่มเติม"}</strong></div>}{confidence !== null && <div className={`calculation-preview ${confidence < 0.9 || mathCheck.amountConsistent === false || mathCheck.weightConsistent === false ? "low-confidence" : ""}`}><span>OCR validation score: {(confidence * 100).toFixed(0)}%</span><strong>{mathCheck.weightConsistent === false ? "น้ำหนักยางสดและเปอร์เซ็นต์ไม่ตรงกับน้ำหนักยางแห้ง" : mathCheck.amountConsistent === false ? "น้ำหนักและราคาไม่ตรงกับยอดเงิน" : confidence < 0.8 ? "ต้องตรวจสอบข้อมูลก่อนสร้างรายการ" : confidence < 0.9 ? "แนะนำให้ตรวจสอบข้อมูลที่อ่านได้" : "คะแนนสูง แต่ควรตรวจสอบก่อนยืนยัน"}</strong></div>}{duplicate.possibleDuplicate && <div className="calculation-preview low-confidence"><span>พบรายการที่อาจซ้ำ {duplicate.matches?.length || 0} รายการ</span><strong>ตรวจสอบก่อนกดยืนยันอีกครั้ง</strong></div>}{error && <div className="form-error" role="alert">{error}</div>}<button className="secondary" onClick={() => void checkDuplicate()} disabled={!garden || busy}>ตรวจรายการซ้ำ</button><button className="primary" onClick={() => void submit()} disabled={!garden || !agreement || !receiptId || !receiptFileId || busy || !fields.saleDate}>{status === "duplicate" ? "ยืนยันว่าเป็นบิลคนละรายการ" : busy ? "กำลังบันทึก..." : "ยืนยันข้อมูลและสร้างรายการ"}</button>{!agreement && <small className="form-hint">ต้องมี ข้อตกลงที่ใช้งานอยู่ ก่อนสร้างรายการขาย</small>}</div></Modal>;
}
...
function AgreementForm({ garden, members, onClose, onSaved }: { garden?: Garden; members: GardenMember[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title="สร้างข้อตกลงแบ่งรายได้" onClose={onClose}><form className="form-grid" onSubmit={submit}><label>คนกรีด (Tapper)<select required value={form.tapperId} onChange={(e) => setForm({ ...form, tapperId: e.target.value })}><option value="">เลือกคนกรีด (Tapper) ในสวน</option>{tappers.map((member) => <option key={member.id} value={member.userId}>{member.name || member.email || member.userId}</option>)}</select></label><label>เริ่มมีผลวันที่<input required type="date" value={form.effectiveFrom} onChange={(e) => setForm({ ...form, effectiveFrom: e.target.value })} /></label><label>สัดส่วนเจ้าของสวน (Owner) (%)<input type="number" min="0" max="100" value={form.ownerPercentage} onChange={(e) => setForm({ ...form, ownerPercentage: e.target.value })} /></label><label>สัดส่วนคนกรีด (Tapper) (%)<input type="number" min="0" max="100" value={form.tapperPercentage} onChange={(e) => setForm({ ...form, tapperPercentage: e.target.value })} /></label><div className="calculation-preview"><span>รวมสัดส่วน</span><strong className={total === 100 ? "" : "form-hint"}>{total}% {total === 100 ? "พร้อมบันทึก" : "ต้องรวมให้ได้ 100%"}</strong></div><button className="primary" disabled={busy || !garden || !form.tapperId || total !== 100}>{busy ? "กำลังบันทึก..." : "สร้างข้อตกลงเวอร์ชัน"}</button>{tappers.length === 0 && <small className="form-hint">ต้องเพิ่มคนกรีด (Tapper) ในหน้า “สวนและแปลง” ก่อนสร้างข้อตกลง</small>}</form></Modal>;
}
