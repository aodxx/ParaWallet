...
function Empty({ text, nextAction, onAction }: { text: string; nextAction?: string; onAction?: () => void }) { return <div className="empty-state"><Leaf size={28} /><strong>{text}</strong><span>{nextAction || "เมื่อมีข้อมูล รายการจะแสดงที่หน้านี้"}</span>{onAction && <button className="secondary" type="button" onClick={onAction}>{nextAction}</button>}</div>; }
