...
function TapperOverview({ data, wallet, connected, salesSeries, salesSeriesMax, onReceipt, onSale, onSettlement, onReports }: { data: DashboardData; wallet: WalletData | null; connected: boolean; salesSeries: number[]; salesSeriesMax: number; onReceipt: () => void; onSale: () => void; onSettlement: () => void; onReports: () => void }) {
  ...
  return <div className="tapper-overview">
    <section className="dual-wallet-priority" aria-label="กระเป๋าเงินคู่">
      <div className="dual-wallet-heading"><span><WalletCards size={21} />กระเป๋าเงินคู่</span><small>ยอดสิทธิที่ตรวจสอบร่วมกัน</small></div>
...
function SettlementsScreen({ settlements, wallet, role, onCreate, onRefresh }: { settlements: Settlement[]; wallet: WalletData | null; role: Role; onCreate: () => void; onRefresh: () => void }) {
  ...
  return <>
    <section className="panel"><div className="panel-head"><div><h2>การส่งเงินและยอดคงค้าง</h2><p>เงินของเจ้าของที่ยังอยู่กับคนกรีด: {money(wallet?.owner.outstanding || 0)}</p></div><div className="panel-actions"><button className="secondary" onClick={onRefresh}>รีเฟรช</button>{role === "tapper" && <button className="primary" onClick={onCreate}><Plus size={16} />บันทึกการส่งเงิน</button>}</div></div>{settlements.length === 0 ? <Empty text="ยังไม่มี settlement ในช่วงนี้" /> : <div className="data-list">{settlements.map((item) => <article className="data-row settlement-row" key={item.id}><div><strong>{money(item.amount)}</strong><span>{item.method === "cash" ? `เงินสด · ${item.location || "ไม่ระบุสถานที่"}` : `โอนธนาคาร${item.bank ? ` · ${item.bank}` : ""}`} · {item.transferDate || "ไม่ระบุวันที่"}</span><small className="evidence-label">{item.method === "bank_transfer" ? <><FileText size={13} />{item.slipFileId ? "มีสลิปแนบ" : "ไม่พบสลิป"}</> : <><Banknote size={13} />รอ Owner ยืนยันการรับเงินสด</>}</small></div><div><span className={`status ${item.status}`}>{labelStatus(item.status)}</span><div className="panel-actions settlement-confirm-actions"><button className={role === "owner" && item.status === "pending_owner_confirmation" ? "primary" : "link-button"} onClick={() => setReviewSettlement(item)}>{role === "owner" && item.status === "pending_owner_confirmation" ? <><ShieldCheck size={15} />ตรวจและยืนยัน</> : <><Eye size={15} />ดูรายละเอียด</>}</button>{role === "tapper" && item.status === "pending_owner_confirmation" && <button className="link-button" onClick={async () => { if (window.confirm("ยกเลิกรายการส่งเงินนี้หรือไม่")) { await api.settlements.cancel(item.id); onRefresh(); } }}>ยกเลิก</button>}</div></div></article>)}</div>}</section>
