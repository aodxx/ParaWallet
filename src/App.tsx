  ...
  return <div className="app-shell">
    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <button className="settings" onClick={() => setShowMobileMore(true)}><Settings2 size={18} />ข้อมูลและการตั้งค่า</button>
      </aside>
    ...
    {reviewSale && <SaleReviewModal sale={reviewSale} agreement={agreements.find((item) => item.id === reviewSale.agreementId)} role={role} onClose={() => setReviewSale(null)} onChanged={(detail, id) => { setReviewSale(null); showCompletion("อัปเดตรายการขายสำเร็จ", detail, "sales", "sale", id); void refresh("sales"); }} />}
...
function CompletionReceipt({ receipt, onClose, onOpen }: { receipt: CompletionReceiptData; onClose: () => void; onOpen: () => void }) { return <div className="completion-backdrop" role="presentation"><section className="completion-receipt" role="dialog" aria-modal="true" aria-labelledby="completion-title"><CheckCircle2 size={34} aria-hidden="true" /><h2 id="completion-title">{receipt.title}</h2><p>{receipt.detail}</p><div className="completion-actions"><button className="secondary" type="button" onClick={onClose}>ปิด</button>{receipt.nextScreen && <button className="primary" type="button" onClick={onOpen}>ดูรายการนี้</button>}</div></section></div>; }
function ReasonDialog({ title, prompt, value, busy, onChange, onCancel, onConfirm, confirmLabel }: { title: string; prompt: string; value: string; busy: boolean; onChange: (value: string) => void; onCancel: () => void; onConfirm: () => void; confirmLabel: string }) { return <div className="completion-backdrop" role="presentation"><section className="reason-dialog" role="dialog" aria-modal="true" aria-labelledby="reason-title"><h2 id="reason-title">{title}</h2><p>กรุณาระบุเหตุผลเพื่อให้ทั้งสองฝ่ายติดตามรายการได้</p><label>{prompt}<textarea autoFocus value={value} onChange={(event) => onChange(event.target.value)} placeholder="กรุณาระบุเหตุผล" required /></label><div className="completion-actions"><button className="secondary" type="button" onClick={onCancel} disabled={busy}>ยกเลิก</button><button className="danger-button" type="button" onClick={onConfirm} disabled={busy || !value.trim()}>{busy ? "กำลังดำเนินการ..." : confirmLabel}</button></div></section></div>; }
...
function SaleReviewModal({ sale, agreement, role, onClose, onChanged }: { sale: Sale; agreement?: Agreement; role: Role; onClose: () => void; onChanged: (detail: string, id: string) => void }) {
...
function GardensScreen({ garden, gardens, members, role, onCreate, onAddMember, onDeactivate }: { garden?: Garden; gardens: Garden[]; members: GardenMember[]; role: Role; onCreate: () => void; onAddMember: () => void; onDeactivate: (member: GardenMember) => void }) {
  return <section className="panel"><div className="panel-head"><div><h2>สวนและแปลง</h2><p>จัดการพื้นที่ จำนวนต้นยาง และสมาชิกที่เข้าถึงข้อมูลสวน</p></div>{role === "owner" && <button className="primary" onClick={onCreate}><Plus size={16} />เพิ่มสวน</button>}</div>{gardens.length === 0 ? <PageState kind="no-garden" title="ยังไม่มีสวนในบัญชีนี้" detail="เพิ่มสวนใหม่เพื่อเริ่มจัดการรายการและสมาชิก" actionLabel={role === "owner" ? "เพิ่มสวน" : undefined} onAction={role === "owner" ? onCreate : undefined} /> : <div className="card-grid">{gardens.map((item) => <div className="info-card" key={item.id}><span className="eyebrow">สวนที่ใช้งานอยู่</span><h3>{item.name}</h3><p>{item.province || "ไม่ระบุจังหวัด"} · {item.district || "ไม่ระบุอำเภอ"}</p><strong>{item.areaRai || 0} ไร่ · {(item.treeCount || 0).toLocaleString()} ต้น</strong></div>)}</div>}{role === "owner" && garden && <section className="member-management" aria-label="จัดการสมาชิกสวน"><div className="member-management-head"><div><span><Users size={19} />สมาชิกสวน</span><small>เฉพาะเจ้าของสวนเท่านั้นที่เพิ่มหรือปิดสิทธิ์คนกรีดได้</small></div><button className="secondary" onClick={onAddMember}><UserPlus size={16} />เพิ่มคนกรีด</button></div>{members.length === 0 ? <PageState kind="empty" title="ยังไม่มีสมาชิกที่ใช้งานอยู่" detail="เพิ่มคนกรีดที่ลงทะเบียนแล้วเพื่อเริ่มรับรายการขาย" actionLabel="เพิ่มคนกรีด" onAction={onAddMember} /> : <div className="member-list">{members.map((member) => <article className="member-row" key={member.id}><div className="member-avatar">{member.role === "owner" ? "O" : "T"}</div><div><strong>{member.name || (member.role === "owner" ? "เจ้าของสวน" : "คนกรีด")}</strong><span>{member.email || member.userId}</span><small>{member.role === "owner" ? "เจ้าของสวน" : "คนกรีด · ใช้งานอยู่"}</small></div>{member.role === "tapper" && <button className="danger-button" onClick={() => onDeactivate(member)}><UserMinus size={15} />ปิดสิทธิ์</button>}</article>)}</div>}</section>}</section>;
}

function AgreementsScreen({ agreements, garden, role, onCreate }: { agreements: Agreement[]; garden?: Garden; role: Role; onCreate: () => void }) { return <section className="panel"><div className="panel-head"><div><h2>ข้อตกลงแบ่งรายได้</h2><p>{garden?.name || "เลือกสวนก่อนสร้างข้อตกลง"} · ทุกเวอร์ชันไม่กระทบรายการย้อนหลัง</p></div>{role === "owner" && <button className="secondary" onClick={onCreate}>สร้างเวอร์ชันใหม่</button>}</div>{agreements.length === 0 ? <PageState kind="empty" title="ยังไม่มีข้อตกลงที่ใช้งานอยู่" detail={role === "owner" ? "สร้างข้อตกลงแบ่งรายได้ก่อนเริ่มรับรายการขาย" : "รอเจ้าของสวนสร้างข้อตกลงเพื่อเริ่มใช้งาน"} actionLabel={role === "owner" ? "สร้างข้อตกลง" : undefined} onAction={role === "owner" ? onCreate : undefined} /> : <div className="data-list">{agreements.map((agreement) => <article className="data-row" key={agreement.id}><div><strong>เวอร์ชัน {agreement.version}</strong><span>มีผล {agreement.effectiveFrom} · {agreement.status}</span></div><div><strong>{agreement.ownerPercentage}/{agreement.tapperPercentage}</strong><span>เจ้าของสวน / คนกรีด</span></div></article>)}</div>}</section>; }
...
function SettlementsScreen({ settlements, wallet, role, focusId, onFocusConsumed, onCreate, onRefresh, onCompleted }: { settlements: Settlement[]; wallet: WalletData | null; role: Role; focusId?: string; onFocusConsumed: () => void; onCreate: () => void; onRefresh: () => void; onCompleted: (detail: string, id: string) => void }) {
  ...
  return <>
    <section className="panel">
      ...
      {settlements.length === 0 ? <PageState kind="empty" title="ยังไม่มีรายการส่งเงิน" detail={role === "tapper" ? "เมื่อส่งเงินให้เจ้าของสวน รายการจะปรากฏที่หน้านี้" : "เมื่อคนกรีดส่งเงิน รายการจะปรากฏที่หน้านี้"} actionLabel={role === "tapper" ? "บันทึกการส่งเงิน" : undefined} onAction={role === "tapper" ? onCreate : undefined} /> : <div className="data-list">{settlements.map((item) =>
...
function PageState({ kind, title, detail, actionLabel, onAction }: { kind: "empty" | "no-garden" | "error" | "unauthorized"; title: string; detail: string; actionLabel?: string; onAction?: () => void }) { return <div className={`empty-state page-state-${kind}`} role={kind === "error" ? "alert" : undefined}><Leaf size={28} /><strong>{title}</strong><span>{detail}</span>{actionLabel && onAction && <button className="secondary" type="button" onClick={onAction}>{actionLabel}</button>}</div>; }
function Empty({ text, nextAction, onAction }: { text: string; nextAction?: string; onAction?: () => void }) { return <PageState kind="empty" title={text} detail={nextAction || "เมื่อมีข้อมูล รายการจะแสดงที่หน้านี้"} actionLabel={onAction ? nextAction : undefined} onAction={onAction} />; }
