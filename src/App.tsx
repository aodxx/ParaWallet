...
import { dateToday, formatThaiDateTime, monthStartToday } from "./date";
...
export default function App() {
  ...
  const deactivateMember = async () => {
    ...
    setDeactivateBusy(true); setActionRetry(null); setActionStatus(createSystemStatus("action", "working", "กำลังปิดสิทธิ์คนกรีด", "ระบบกำลังตรวจสอบงานที่ยังไม่จบและเงินเจ้าของที่ยังเหลือ", { nextAction: "รอสักครู่" }));
    try {
...
function screenDescription(screen: Screen, role: Role) {
  if (screen === "overview") return role === "owner" ? "ภาพรวมส่วนแบ่งสะสม งานรอตรวจ และเงินเจ้าของคงเหลือ" : "รายได้สะสมของคุณ เงินเจ้าของที่ถืออยู่ และงานสำคัญวันนี้";
  return ({
...
function Overview({ data, wallet, role, connected, onSale, onReceipt, onSettlement, onReviewSales, onReviewSettlements, onReports }: { data: DashboardData; wallet: WalletData | null; role: Role; connected: boolean; onSale: () => void; onReceipt: () => void; onSettlement: () => void; onReviewSales: () => void; onReviewSettlements: () => void; onReports: () => void }) {
  ...
  return <div className="owner-overview">
    {Boolean(wallet?.testDataCount) && <section className="test-data-notice"><ShieldCheck size={18} /><span><strong>แยกข้อมูลทดสอบแล้ว {wallet?.testDataCount} รายการ</strong><small>รายการทดสอบถูกเก็บไว้ตรวจย้อนหลัง แต่ไม่รวมในยอดเงินจริงด้านล่าง</small></span></section>}
    <section className="pending-work" aria-label="งานที่เจ้าของต้องตรวจสอบ"><div><strong>งานที่ต้องทำ</strong><span>ตรวจหลักฐานก่อนยืนยันรายการขายและการรับเงิน</span></div><div className="pending-work-actions"><button onClick={onReviewSales}><FileText size={18} /><span>รายการขายรอตรวจ</span><strong>{data.pendingSales || 0}</strong></button><button onClick={onReviewSettlements}><WalletCards size={18} /><span>เงินที่คนกรีดส่งมารอยืนยัน</span><strong>{data.pendingSettlements || 0}</strong></button></div></section>
    <section className="owner-wallet-pair" aria-label="ยอดเงินจริงสะสมของเจ้าของ"><article><span><small>ส่วนแบ่งสะสมที่ยืนยันแล้ว</small><WalletCards size={21} /></span><strong>{showMoney(owner)}</strong><p>รวมทุกบิลจริงตั้งแต่เริ่มใช้งาน</p></article><article><span><small>เจ้าของได้รับสะสม</small><ShieldCheck size={21} /></span><strong>{showMoney(wallet?.owner.totalReceived || 0)}</strong><p>รวมรายการส่งเงินที่ยืนยันแล้ว</p></article></section>
    <section className="financial-proof" aria-label="วิธีตรวจสอบยอดคงเหลือ"><div><small>ส่วนแบ่งเจ้าของสะสม</small><strong>{showMoney(owner)}</strong></div><span>−</span><div><small>ได้รับแล้วสะสม</small><strong>{showMoney(wallet?.owner.totalReceived || 0)}</strong></div><span>=</span><div className="proof-result"><small>เงินเจ้าของคงเหลือทุกบิล</small><strong>{showMoney(outstanding)}</strong></div></section>
    <section className="owner-summary-grid"><Metric label="งานรอตรวจสอบ" value={connected ? `${data.pendingReviews} รายการ` : "—"} icon={<FileText />} /><Metric label="ยอดขายเดือนนี้" value={showMoney(data.monthlySales)} icon={<Sprout />} /></section><section className="owner-sales-card sales-card"><div><small>ยอดขายรวมเดือนนี้</small><strong>{showMoney(data.monthlySales)}</strong><span className="growth">{connected ? "เฉพาะบิลจริงที่ยืนยันแล้วในเดือนนี้" : "รอเชื่อมต่อฐานข้อมูล"}</span></div><div className="bars">{connected && salesSeries.some((value) => value > 0) ? salesSeries.map((value, i) => <div key={i} style={{ height: `${Math.max(8, value / salesSeriesMax * 100)}%` }}><span>ส.{i + 1}</span></div>) : <p className="empty-chart">{connected ? "ยังไม่มียอดขายที่ยืนยันในเดือนนี้" : "กราฟจะแสดงเมื่อโหลดข้อมูลจริงสำเร็จ"}</p>}</div></section><section className="quick-actions"><button onClick={onReports}><FileDown size={18} />ดูรายงาน</button></section>
  </div>;
}
...
function TapperOverview({ data, wallet, connected, salesSeries, salesSeriesMax, onReceipt, onSettlement, onReports }: { data: DashboardData; wallet: WalletData | null; connected: boolean; salesSeries: number[]; salesSeriesMax: number; onReceipt: () => void; onSettlement: () => void; onReports: () => void }) {
  ...
  const ownerEntitlement = wallet?.tapper.ownerEntitlement ?? 0;
  ...
  return <div className="tapper-overview">
    {Boolean(wallet?.testDataCount) && <section className="test-data-notice"><ShieldCheck size={18} /><span><strong>แยกข้อมูลทดสอบแล้ว {wallet?.testDataCount} รายการ</strong><small>รายการทดสอบไม่รวมในยอดเงินจริงของคุณและเจ้าของสวน</small></span></section>}
    <section className="dual-wallet-priority" aria-label="กระเป๋าเงินคู่">
      ...
      <div className="dual-wallet-grid">
        <article className="wallet-tapper"><small>รายได้สะสมของฉัน</small><strong>{showMoney(tapperIncome)}</strong><span>รวมทุกบิลจริงที่ยืนยันแล้ว</span></article>
        <article className="wallet-owner"><small>เงินเจ้าของคงเหลือทุกบิล</small><strong>{showMoney(ownerMoneyHeld)}</strong><span>รอส่งหรือรอเจ้าของยืนยัน</span></article>
      </div>
      <div className="wallet-transferred"><ShieldCheck size={16} /><span>ส่งให้เจ้าของและยืนยันแล้ว (สะสม)</span><strong>{showMoney(ownerMoneyTransferred)}</strong></div>
    </section>
    <section className="financial-proof" aria-label="วิธีตรวจสอบเงินเจ้าของที่คงเหลือ"><div><small>ส่วนแบ่งเจ้าของสะสม</small><strong>{showMoney(ownerEntitlement)}</strong></div><span>−</span><div><small>ส่งและยืนยันแล้ว</small><strong>{showMoney(ownerMoneyTransferred)}</strong></div><span>=</span><div className="proof-result"><small>เงินเจ้าของคงเหลือ</small><strong>{showMoney(ownerMoneyHeld)}</strong></div></section>
    ...
    <section className="tapper-sales-card sales-card"><div><small>ยอดขายที่ยืนยันแล้วในเดือนนี้</small><strong>{showMoney(data.monthlySales)}</strong><span className="growth">{connected ? "แยกตามสัปดาห์ของเดือนปัจจุบัน" : "รอเชื่อมต่อฐานข้อมูล"}</span></div><div className="bars">{connected && salesSeries.some((value) => value > 0) ? salesSeries.map((value, i) => <div key={i} style={{ height: `${Math.max(8, value / salesSeriesMax * 100)}%` }}><span>ส.{i + 1}</span></div>) : <p className="empty-chart">{connected ? "ยังไม่มียอดขายที่ยืนยันในเดือนนี้" : "กราฟจะแสดงเมื่อโหลดข้อมูลจริงสำเร็จ"}</p>}</div></section>
  </div>;
}
...
function AgreementsScreen({ agreements, garden, role, onCreate }: { agreements: Agreement[]; garden?: Garden; role: Role; onCreate: () => void }) {
  return <section className="panel"><div className="panel-head"><div><h2>ข้อตกลงแบ่งรายได้</h2><p>{garden?.name || "เลือกสวนก่อนสร้างข้อตกลง"} · เวอร์ชันใหม่ใช้เฉพาะบิลหลังวันที่เริ่มมีผล</p></div>{role === "owner" && <button className="secondary" onClick={onCreate}>สร้างข้อตกลงใหม่</button>}</div>{agreements.length === 0 ? <PageState title="ยังไม่มีข้อตกลงสำหรับใช้งานจริง" detail={role === "owner" ? "ข้อมูลทดสอบเดิมถูกแยกออกแล้ว กรุณาสร้างข้อตกลงจริงก่อนให้คนกรีดบันทึกบิล" : "รอเจ้าของสวนสร้างข้อตกลงจริงก่อน จึงจะบันทึกรายการขายได้"} actionLabel={role === "owner" ? "สร้างข้อตกลงจริง" : undefined} onAction={role === "owner" ? onCreate : undefined} /> : <div className="data-list">{agreements.map((agreement, index) => <article className={`data-row agreement-row ${agreement.status === "active" ? "active" : "history"}`} key={agreement.id}><div><strong>{agreement.status === "active" ? "ข้อตกลงที่ใช้อยู่" : "ประวัติข้อตกลง"} · เวอร์ชัน {agreement.version}</strong><span>เริ่มใช้ {formatThaiDateTime(agreement.effectiveFrom)} · {agreement.status === "active" ? "ใช้กับบิลใหม่" : "ไม่ใช้กับบิลใหม่แล้ว"}</span></div><div><strong>{agreement.ownerPercentage}% / {agreement.tapperPercentage}%</strong><span>เจ้าของสวน / คนกรีด{index === 0 && agreement.status === "active" ? " · เวอร์ชันล่าสุด" : ""}</span></div></article>)}</div>}</section>;
}
...
function SettlementsScreen({ settlements, wallet, role, focusedSettlementId, onFocusHandled, onCreate, onRefresh, onCompleted }: { settlements: Settlement[]; wallet: WalletData | null; role: Role; focusedSettlementId?: string; onFocusHandled: () => void; onCreate: () => void; onRefresh: () => void; onCompleted: (receipt: CompletionReceiptData) => void }) {
  ...
  const visibleOutstanding = role === "tapper" ? wallet?.tapper.ownerMoneyHeld : wallet?.owner.outstanding;
  useEffect(() => {
  ...
  const cancelSettlement = async (item: Settlement) => {
    ...
    try {
      ...
      onCompleted({ title: "ยกเลิกรายการส่งเงินแล้ว", detail: "รายการยังอยู่ในประวัติ และเงินเจ้าของคงเหลือไม่เปลี่ยน", statusLabel: "ยกเลิกแล้ว", rows: [{ label: "จำนวนเงิน", value: money(item.amount) }, { label: "เงินเจ้าของคงเหลือ", value: "ไม่เปลี่ยน" }], nextScreen: "settlements", entityType: "settlement", entityId: item.id });
      onRefresh();
    }
    catch (caught) { setError(userMessageForApiError(caught)); }
  ...
  return <>
    <section className="panel">
      <div className="panel-head">
        <div><h2>การส่งเงินและยอดคงเหลือ</h2><p>เงินเจ้าของคงเหลือทุกบิลจริง: {money(visibleOutstanding || 0)}</p></div>
    ...
    {reviewSettlement && <SettlementReviewModal settlement={reviewSettlement} outstandingBefore={visibleOutstanding} role={role} onClose={() => setReviewSettlement(null)} onChanged={(receipt) => { setReviewSettlement(null); onCompleted(receipt); onRefresh(); }} />}
...
function SettlementReviewModal({ settlement, outstandingBefore, role, onClose, onChanged }: { settlement: Settlement; outstandingBefore?: number; role: Role; onClose: () => void; onChanged: (receipt: CompletionReceiptData) => void }) {
  ...
  const confirmSettlement = async () => {
    ...
    setBusy(true); setError("");
    try {
      const confirmed = await api.settlements.confirm(settlement.id);
      const rows = [{ label: "ยอดที่ได้รับ", value: money(settlement.amount) }];
      if (confirmed.allocations?.length) rows.push({ label: "นำไปตัดยอด", value: `${confirmed.allocations.length} บิล` });
      if (outstandingBefore !== undefined) {
        rows.push({ label: "เงินเจ้าของคงเหลือก่อนยืนยัน", value: money(outstandingBefore) });
        rows.push({ label: "เงินเจ้าของคงเหลือใหม่", value: money(Math.max(0, outstandingBefore - settlement.amount)) });
      } else rows.push({ label: "ผลต่อเงินเจ้าของคงเหลือ", value: `ลดลง ${money(settlement.amount)}` });
      onChanged({ title: "ยืนยันการรับเงินสำเร็จ", detail: "บันทึกการรับเงินจริงแล้ว และลดเงินเจ้าของคงเหลือตามจำนวนที่ยืนยัน", statusLabel: "ยืนยันรับเงินแล้ว", rows, nextScreen: "settlements", entityType: "settlement", entityId: settlement.id });
    }
    catch (caught) { setError(userMessageForApiError(caught)); }
    finally { setBusy(false); }
  };
  const rejectSettlement = async () => {
    ...
    try {
      ...
      setReasonOpen(false);
      onChanged({ title: "ปฏิเสธรายการส่งเงินแล้ว", detail: "แจ้งเหตุผลให้คนกรีดแล้ว เงินเจ้าของคงเหลือไม่เปลี่ยน", statusLabel: "ปฏิเสธแล้ว", rows: [{ label: "จำนวนเงิน", value: money(settlement.amount) }, { label: "เงินเจ้าของคงเหลือ", value: outstandingBefore === undefined ? "ไม่เปลี่ยน" : `${money(outstandingBefore)} (ไม่เปลี่ยน)` }], nextScreen: "settlements", entityType: "settlement", entityId: settlement.id });
    }
    catch (caught) { setError(userMessageForApiError(caught)); }
    finally { setBusy(false); }
  };
...
function ReportsScreen({ garden }: { garden?: Garden }) {
  ...
  return <section className="panel"><div className="panel-head"><div><h2>รายงานตามช่วงเวลา</h2><p>ยอดขายและส่วนแบ่งใช้ช่วงวันที่ที่เลือก ส่วนเงินเจ้าของคงเหลือเป็นยอดปัจจุบันทุกบิล</p></div><div className="report-download"><button className="secondary" disabled={!report || report.rows.length === 0 || busy} onClick={exportCsv}>ดาวน์โหลดตาราง</button>{(!report || report.rows.length === 0) && <small>โหลดรายงานที่มีรายการก่อน จึงจะดาวน์โหลดตารางได้</small>}</div></div><div className="filters"><label>ตั้งแต่<input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label>ถึง<input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label><button className="primary" disabled={busy || !garden} onClick={() => void load()}>{busy ? "กำลังโหลดรายงาน..." : "ดูรายงาน"}</button></div>{error && <div className="form-error report-error" role="alert"><strong>โหลดรายงานไม่สำเร็จ</strong><span>{error}</span><button className="secondary" type="button" onClick={() => void load()} disabled={busy}>ลองใหม่</button></div>}{report && report.rows.length > 0 ? <div className="report-grid">{[["ยอดขายในช่วงที่เลือก", money(Number(report.summary.grossSales))], ["ส่วนเจ้าของในช่วงที่เลือก", money(Number(report.summary.ownerShare))], ["ส่วนคนกรีดในช่วงที่เลือก", money(Number(report.summary.tapperShare))], ["เงินเจ้าของคงเหลือปัจจุบัน (ทุกบิล)", money(Number(report.summary.outstanding))]].map(([label, value]) => <div className="metric" key={label}><small>{label}</small><strong>{value}</strong></div>)}</div> : report ? <PageState title="ไม่พบรายการในช่วงนี้" detail="ลองขยายช่วงวันที่ แล้วกดดูรายงานอีกครั้ง" actionLabel="โหลดรายงานอีกครั้ง" onAction={() => void load()} /> : !error && <PageState title="เลือกระยะเวลาของรายงาน" detail="กำหนดวันที่เริ่มและสิ้นสุด แล้วกดดูรายงาน" tone="info" />}</section>;
}
